...
int
#ifdef __APPLE__
 zfs_dirlook(znode_t *dzp, char *name, u_long namehash, struct vnode **vpp)
#else
...
#endif
{
	...
	if (name[0] == 0 || (name[0] == '.' && name[1] == 0)) {
		...
		*vpp = zfsctl_root(dzp);
		...
		error = zfs_dirent_lock(&dl, dzp, &cn, &zp, ZEXISTS | ZSHARED);
#else
...
#ifdef __APPLE__
	/* For OSX, determine the proper dtype to store for the object */
