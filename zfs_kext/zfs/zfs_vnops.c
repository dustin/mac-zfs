...
/*
 * Until generic kqueue support is part of VFS, we'll need
 * to implement support for it in the zfs file system.
 */
#ifdef ZFS_KQUEUES

#define ZFS_KNOTE(ZP, HINT)                                   \
	do {                                                  \
		if (!(SLIST_EMPTY(&ZP->z_knotes))) {          \
			/* Note: z_lock protects the list */  \
			mutex_enter(&ZP->z_lock);             \
			KNOTE(&ZP->z_knotes, (HINT));         \
			mutex_exit(&ZP->z_lock);              \
		}                                             \
	} while (0)

#define ZFS_KNOTE_LOCKED(ZP, HINT)                            \
	do {                                                  \
		if (!(SLIST_EMPTY(&ZP->z_knotes))) {          \
			KNOTE(&ZP->z_knotes, (HINT));         \
		}                                             \
 ...
 */
static int
mappedread(struct vnode *vp, char *addr, int nbytes, struct uio *uio)
{
...
zfs_vnop_read(struct vnop_read_args *ap)
{
	...
	if (MANDMODE((mode_t)zp->z_phys->zp_mode)) {
		if (error = chklock(vp, FREAD,
		    uio_offset(uio), uio_resid(uio), uio->uio_fmode, ct)) {
			ZFS_EXIT(zfsvfs);
			return (error);
		}
	}
	...
	for (ndone = 0; ndone < cnt; ndone += zfs_read_chunk_size) {
		 ...
		 * with the uio buffer.
		 */
		delta = uio_offset(uio) - dbpp[0]->db_offset;
		...
		for (i = 0; i < numbufs; i++) {
			...
			if (vn_has_cached_data(vp)) {
				...
				error = uio_move((caddr_t)dbp->db_data + delta,
...
zfs_vnop_lookup(struct vnop_lookup_args *ap)
{
	...
	*vpp = NULL;
	...
	error = zfs_dirlook(zdp, filename, vpp);
...
#ifdef ZFS_KQUEUES
	ZFS_KNOTE(dzp, NOTE_ATTRIB | NOTE_WRITE);
#endif
out:
...
top:
	*vpp = NULL;
	...
	dirname = cnp->cn_nameptr;
	ASSERT(dirname[cnp->cn_namelen] == '\0');
...
#ifdef ZFS_KQUEUES
	ZFS_KNOTE(dzp, NOTE_WRITE | NOTE_LINK | NOTE_ATTRIB);
	ZFS_KNOTE(zp, NOTE_DELETE);
#endif
out:
	if (dl) {
...
#ifdef ZFS_KQUEUES
		if (vap->va_data_size > pzp->zp_size)
			notehint |= NOTE_EXTEND;
#endif
		/*
...
#ifdef ZFS_KQUEUES
	ZFS_KNOTE(szp, NOTE_RENAME);
	ZFS_KNOTE(sdzp, NOTE_ATTRIB | NOTE_WRITE);
	if (sdvp != tdvp)
		ZFS_KNOTE(tdzp, NOTE_ATTRIB | NOTE_WRITE);
#endif
out:
	if (zl != NULL)
		zfs_rename_unlock(&zl);
...
zfs_vnop_pagein(struct vnop_pagein_args *ap)
{
	...
	if (off >= zp->z_phys->zp_size) {
		ZFS_EXIT(zfsvfs);
		if (!(flags & UPL_NOCOMMIT))
			ubc_upl_abort_range(pl, pl_offset, len,
	...
	if (!(flags & UPL_NOCOMMIT)) {
		if (error)
			ubc_upl_abort_range(pl, pl_offset, ap->a_size,
					    UPL_ABORT_ERROR |
					    UPL_ABORT_FREE_ON_EMPTY);
		else
			ubc_upl_commit_range(pl, pl_offset, ap->a_size,
...
zfs_vnop_pageout(struct vnop_pageout_args *ap)
{
	...
	if (off > filesz) {
		/* past end of file */
		if (!(flags & UPL_NOCOMMIT))
			ubc_upl_abort_range(pl, pl_offset, len, UPL_ABORT_FREE_ON_EMPTY);
		ZFS_EXIT(zfsvfs);
		return (0);
	}
...
top:
	...
	if (/*zp->z_blksz*/ len <= PAGESIZE) {
		...
		ubc_upl_map(pl, (vm_offset_t)&va);
		va += pl_offset;
		dmu_write(zfsvfs->z_os, zp->z_id, off, len, va, tx);
		ubc_upl_unmap(pl);
	} else {
		error = dmu_write_pages(zfsvfs->z_os, zp->z_id, off, len, pl, tx);
	}
...
zfs_vnop_getxattr(struct vnop_getxattr_args *ap)
{
	...
	if ( (error = zfs_dirlook(VTOZ(xdvp), (char *)ap->a_name, &xvp)) ) {
		if (error == ENOENT)
			error = ENOATTR;
		goto out;
	}
...
top:
	...
	if ( (error = zfs_dirent_lock(&dl, dzp, name, &xzp, flag)) ) {
		goto out;
	}
	...
	zfs_log_create(zilog, tx, TX_CREATE, dzp, xzp, name);
	dmu_tx_commit(tx);
	zfs_dirent_unlock(dl);
out:
	if (error == EEXIST)
...
zfs_vnop_removexattr(struct vnop_removexattr_args *ap)
{
	...
	cn.cn_nameptr = ap->a_name;
	cn.cn_pnlen = strlen(cn.cn_nameptr);
...
zfs_vnop_getnamedstream(struct vnop_getnamedstream_args* ap)
{
	...
	*svpp = NULLVP;
	...
	/* Lookup the attribute name. */
	if ( (error = zfs_dirlook(VTOZ(xdvp), (char *)ap->a_name, svpp)) ) {
		if (error == ENOENT)
			error = ENOATTR;
	}
out:
...
#ifdef ZFS_KQUEUES
	ZFS_KNOTE(fzp, NOTE_ATTRIB);
	ZFS_KNOTE(tzp, NOTE_ATTRIB);
#endif
	ZFS_EXIT(zfsvfs);
...
static void
filt_zfsdetach(struct knote *kn)
{
	struct vnode *vp;
	znode_t *zp;
	...
	zp = VTOZ(vp);
	mutex_enter(&zp->z_lock);
	(void) KNOTE_DETACH(&zp->z_knotes, kn);
	mutex_exit(&zp->z_lock);
	vnode_put(vp);
}
...
/*ARGSUSED*/
static int
filt_zfsread(struct knote *kn, long hint)
{
	...
	if (!(kn->kn_flags & EV_POLL)) {
		off_t amount;
 		...
 		/*
 		 * XXX - no KPI to get at the file offset for kn->kn_fp
 		 */
		amount = VTOZ(vp)->z_phys->zp_size /* - kn->kn_fp->f_fglob->fg_offset */;
...
/*ARGSUSED*/
static int
filt_zfswrite(struct knote *kn, long hint)
{
	struct vnode *vp = (struct vnode *)kn->kn_hook;
...
static int
filt_zfsvnode(struct knote *kn, long hint)
{
	struct vnode *vp = (struct vnode *)kn->kn_hook;
...
static struct filterops zfsread_filtops = 
	{ 1, NULL, filt_zfsdetach, filt_zfsread };
static struct filterops zfswrite_filtops = 
	{ 1, NULL, filt_zfsdetach, filt_zfswrite };
static struct filterops zfsvnode_filtops = 
	{ 1, NULL, filt_zfsdetach, filt_zfsvnode };

/*
 * Add a kqueue filter.
 */
static int
zfs_vnop_kqfiltadd(struct vnop_kqfilt_add_args *ap)
{
	struct vnode *vp = ap->a_vp;
	...
	mutex_enter(&zp->z_lock);
	(void) KNOTE_ATTACH(&zp->z_knotes, kn);
	mutex_exit(&zp->z_lock);
exit:
	ZFS_EXIT(zfsvfs);
	return (error);
 ...
 */
static int
zfs_vnop_kqfiltremove(struct vnop_kqfilt_remove_args *ap)
{
	return (ENOTSUP);
}
#endif /* ZFS_KQUEUES */
...
#ifdef ZFS_KQUEUES
	{&vnop_kqfilt_add_desc,		(VOPFUNC)zfs_vnop_kqfiltadd},
	{&vnop_kqfilt_remove_desc,	(VOPFUNC)zfs_vnop_kqfiltremove},
#endif
	{NULL, (VOPFUNC)NULL }
};
