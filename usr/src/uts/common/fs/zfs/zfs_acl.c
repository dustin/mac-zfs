...
#ifndef __APPLE__ // on Mac OS X, the change of ACL does not change the mode bits
...
#ifdef __APPLE__
int
zfs_getacl(znode_t *zp, struct kauth_acl **aclpp, cred_t *cr)
{
	zfs_acl_t	*aclp;
	kauth_acl_t  k_acl;
...
#ifdef __APPLE__
int
zfs_setacl(znode_t *zp, struct kauth_acl *k_acl, cred_t *cr, dmu_tx_t  *tx)
{
	zfsvfs_t	*zfsvfs = zp->z_zfsvfs;
	...
	/*
	 * Note that under Mac OS X, zfs_setacl() is called by
	 * zfs_vnop_setattr() and the transaction is ready to
	 * go including the dmu_tx_hold_write() call.  So we
	 * don't need to worry about the tx setup here.
	 */
	aclp = zfs_acl_alloc(aclcnt);
 	/*
	 * Translate Mac OS X ACEs to Open Solaris ACEs
	 */
	for (i = 0; i < aclcnt; i++) {
		/* Note Mac OS X GUID is a 128-bit identifier */
		guidp = &k_acl->acl_ace[i].ace_applicable;
		...
		aclp->z_acl[i].a_access_mask = mask;
		...
		aclp->z_acl[i].a_type = type;
	}
	if (!zfs_acl_valid(zp, aclp->z_acl, aclcnt, &inherit)) {
		error = EINVAL;
		goto exit;
	}
	mutex_enter(&zp->z_lock);
	...
	zfs_log_acl(zilog, tx, TX_ACL, zp, aclcnt, aclp->z_acl);
